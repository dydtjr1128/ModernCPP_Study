...
int main() {
	std::thread th([]() {std::cout << "hello!!" << std::endl; });
	//th.detach();
	th.join();
	std::mutex mutex;
	...
	try {
		std::thread th1([&]() {
			std::cout << "th1 lock" << std::endl;
			mutex.lock();
			std::cout << "th1 lock release" << std::endl;
			});

		std::thread th2([&]() {
			std::cout << "th2 lock" << std::endl;
			lock.try_lock();
			std::cout << "th2 lock release" << std::endl;
			});
		th1.join();
		th2.join();
	}
